...
Vue.component('cart', {
    ...
    template: `
    <li class="li header__basket"  >    
        <a class="header__basket" @click="showCart = !showCart" >
...
Vue.component('cart-item', {
    props: ['cartItem'],
    template: `
            <article class="basketRow">
            <div class="basketRow__img">
                <img :src="cartItem.imgProduct" alt="Cart Product Image">
            </div>
            <div class="basketRow_info">
                <h3>{{ cartItem.product_name }}</h3>
                <p>Quantity: {{ cartItem.quantity }}</p>
                <p>Price per Item: $ {{ cartItem.price }}</p>
                <p class="basketRow_info_price">Total Price: $ {{ cartItem.quantity * cartItem.price }}</p>
            </div>
            <div>
            <button class="remove" @click="$emit('remove', cartItem)">Remove</button></div>
        </article>           
